...
def vit(cfg):
    return ViT(
        ...
        flash_attn=cfg.MODEL.BACKBONE.get("FLASH_ATTN", True),
    )
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        if flash_attn:
            if not Block._flash_printed:
                print(f"[ViT Block] Using FlashAttention (flash_attn library)")
                Block._flash_printed = True
            ...
            if not Block._standard_printed:
                print(f"[ViT Block] Using standard Attention (manual matmul, NO Flash Attention)")
                Block._standard_printed = True
            self.attn = Attention(
                dim,
                num_heads=num_heads,
                qkv_bias=qkv_bias,
