...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if x_mask is not None:
            # Ensure x_mask is on the same device as x
        ...
        if self.enable_twoway:
            ...
            attn_mask = None
            if x_mask is not None:
                # Ensure x_mask is on the same device as context
                x_mask_dev = x_mask.to(context.device)
                attn_mask = (x_mask_dev[:, None, :].repeat(1, context.shape[1], 1)) > 0
