...
class PromptEncoder(nn.Module):
    def __init__(
        ...
        # Optimization: use a single Embedding instead of ModuleList to avoid for-loop
        # Indices: 0~num_body_joints-1 = each joint, num_body_joints = invalid (-2), num_body_joints+1 = not_a_point (-1)
        self.all_point_embeddings = nn.Embedding(num_body_joints + 2, embed_dim)
        # Keep the old interface for compatibility with saved models
        ...
        # Flag indicating whether weights have been synced
    ...
    def _sync_embeddings(self):
        """Sync old ModuleList weights to the new merged Embedding (only needed when loading old models)."""
        if self._embeddings_synced:
            return
        with torch.no_grad():
            # Copy each joint's embedding
            for i in range(self.num_body_joints):
                self.all_point_embeddings.weight[i] = self.point_embeddings[i].weight.squeeze(0)
            # Copy invalid (-2) and not_a_point (-1) embeddings
            self.all_point_embeddings.weight[self.num_body_joints] = self.invalid_point_embed.weight.squeeze(0)
            self.all_point_embeddings.weight[self.num_body_joints + 1] = self.not_a_point_embed.weight.squeeze(0)
    ...
    def _embed_keypoints(
        ...
        # Ensure weights have been synced
        if not self._embeddings_synced:
            self._sync_embeddings()

        # Positional encoding
        point_embedding = self.pe_layer._pe_encoding(points.to(torch.float))

        # Optimization: use vectorized operations instead of for-loop
        # Map labels to all_point_embeddings indices:
        # -2 (invalid) -> num_body_joints
        # -1 (not_a_point) -> num_body_joints + 1
        # 0~num_body_joints-1 -> unchanged
        label_indices = labels.clone()
        label_indices[labels == -2] = self.num_body_joints
        label_indices[labels == -1] = self.num_body_joints + 1

        # Fetch all embeddings at once (vectorized)
        joint_embeddings = self.all_point_embeddings(label_indices.long())

        # invalid points (-2): zero out positional encoding, keep only joint embedding
        # not_a_point (-1): keep positional encoding, add joint embedding
        point_embedding[labels == -2] = 0.0

        # Add joint embedding
        point_embedding = point_embedding + joint_embeddings
