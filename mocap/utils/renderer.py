...
class Renderer:
    ...
    def __init__(self, focal_length, faces=None):
        ...
        self._offscreen_w = None
        self._offscreen_h = None

    def _get_offscreen_renderer(self, w, h):
        if self._offscreen_renderer is None or self._offscreen_w != w or self._offscreen_h != h:
            if self._offscreen_renderer is not None:
                self._offscreen_renderer.delete()
            self._offscreen_renderer = pyrender.OffscreenRenderer(viewport_width=w, viewport_height=h)
            self._offscreen_w = w
            self._offscreen_h = h
    ...
    def __call__(
        ...
        renderer = self._get_offscreen_renderer(w, h)
