...
def visualize_sample_together(img_cv2, outputs, faces):
    ...
    for pid, person_output in enumerate(outputs_sorted):
        ...
        if vertices is None or len(vertices) == 0 or np.any(np.isnan(vertices)) or np.any(np.isnan(cam_t)):
            print(f"  [WARNING] Person {pid} has invalid vertices, skipping mesh rendering")
            continue
        all_pred_vertices.append(vertices + cam_t)
        all_faces.append(faces + len(vertices) * valid_vertex_count)
        valid_vertex_count += 1
    # Handle case where no valid vertices exist
    if len(all_pred_vertices) == 0:
        print("  [WARNING] No valid vertices for any person, skipping mesh rendering")
        img_mesh = img_cv2.copy()
